...
public class PlanoDeSaudeControllerAPI {
    ...
    public ResponseEntity<List<PlanoDeSaude>> getAll() {
        var listaPlanoSaude = service.getAll();
        return new ResponseEntity<List<PlanoDeSaude>>(listaPlanoSaude, HttpStatus.OK);
    }
    ...
    @PostMapping
    public ResponseEntity<PlanoDeSaude> insertPlanoDeSaude(@RequestBody PlanoDeSaude planoDeSaude) {
    ...
    public ResponseEntity<List<PlanoDeSaude>> getByNome(@PathVariable("nome") String nome) {
        var listaPlanoSaude = service.getByName(nome);
        return new ResponseEntity<List<PlanoDeSaude>>(listaPlanoSaude, HttpStatus.OK);
    }

    @PutMapping("/{id}")
    public ResponseEntity<PlanoDeSaude> update(@PathVariable("id") long id, @RequestBody PlanoDeSaude planoDeSaude) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<PlanoDeSaude> delete(@PathVariable("id") long id) {
        var planoDeSaudeAntigo = service.findById(id);
        if (planoDeSaudeAntigo == null) {
            return ResponseEntity.notFound().build();
        }
