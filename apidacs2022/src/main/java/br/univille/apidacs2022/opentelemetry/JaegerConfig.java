...
public class JaegerConfig {
    ...
    public JaegerTracer jaegerTracer() {
        ...
        var sampler = new SamplerConfiguration().withType(RateLimitingSampler.TYPE).withParam(10);

        var reporter = new ReporterConfiguration().withLogSpans(true)
                .withSender(new SenderConfiguration().withAgentHost("172.18.0.35"));

        return new io.jaegertracing.Configuration("dacsapi-brunouliana").withSampler(sampler).withReporter(reporter)
                .getTracer();
